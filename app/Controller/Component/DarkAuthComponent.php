...
class DarkAuthComponent extends Object { 
	...
	var $user_name_field = 'username'; //e.g. email or firstname or username... 
	...
	*/ 
	function hasher($plain_text){
		...
		$crypt = crypt($plain_text, '$2a$' . $cost . '$' . $salt);
		return $crypt;
  ...
  function setCookieInfo($data,$expiry=0){ 
	  if($data === false){ 
		...
		if($_SERVER['SERVER_NAME']=='localhost'){ 
		  ...
		  $domain = '.'.$_SERVER['SERVER_NAME']; // doesn't handle subdomains well?
		  exit;
		} 
  ...
  function destroyData(){ 
	...
	$this->Session->delete('DarkAuth.service_auth_user'); 
	
	if($this->allow_cookie){ 
		// By default does not fire
